...
contract_transactions(_Config) ->    % miner has an account
    ...
    %% Call objects
    {ok, 200, #{<<"tx">> := SpendTx}} = post_spend_tx(aec_base58c:encode(account_pubkey, MinerPubkey), 1, 1),
...
nameservice_transaction_claim(MinerAddress, MinerPubkey) ->
    ...
    sign_and_post_tx(PreclaimTxEnc),
...
naming_system_manage_name(_Config) ->
    ...
    {ok, 200, #{<<"tx">> := SpendTx}} = post_spend_tx(EncodedNHash, 77, 50),
    sign_and_post_tx(SpendTx),
    aecore_suite_utils:mine_blocks(aecore_suite_utils:node_name(?NODE), 3),
    {ok, []} = rpc(aec_tx_pool, peek, [infinity]),
